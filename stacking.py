...
for train_index, val_index in kf.split(scaled_X_train):
    X_kf_train, X_kf_val = scaled_X_train[train_index], scaled_X_train[val_index]
    y_kf_train, y_kf_val = y_train.iloc[train_index], y_train.iloc[val_index]

    # Huấn luyện mô hình Stacking
    stacking_model.fit(X_kf_train, y_kf_train)

    # Dự đoán trên tập validation
    y_kf_val_pred = stacking_model.predict(X_kf_val)

    # Tính toán R² và MSE cho tập validation
    r2 = r2_score(y_kf_val, y_kf_val_pred)
    mse = mean_squared_error(y_kf_val, y_kf_val_pred)
    fold_results.append((r2, mse))

# Hiển thị kết quả k-fold
mean_r2 = np.mean([result[0] for result in fold_results])
mean_mse = np.mean([result[1] for result in fold_results])
