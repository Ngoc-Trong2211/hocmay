...
validation_metrics = evaluate_model(validation_predictions, means, stds, data_label="Validation")
...
def predictBMI_real(data):
    ...
    df = predict_BMI(df)
    df['BMI'] = np.nan  # Thêm cột BMI với giá trị NaN cho dữ liệu mới
...
# Đánh giá trên tập dữ liệu mới (testing)
# Không đánh giá cho dữ liệu người dùng vì không có giá trị BMI thực
print("Dữ liệu mới đã được dự đoán. Kết quả:")
